      ...
      <header className="sticky top-0 z-50 bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-800/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            <Link to="/cart" className="text-sm font-medium hover:text-blue-600 flex items-center">
              Корзина {cart.length > 0 && <span className="ml-1 bg-blue-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">{cart.length}</span>}
            </Link>
            ...
            {token && <Link to="/admin" className="text-sm font-medium hover:text-blue-600">Админ</Link>}
            {token ? (
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.to} to={item.to} className={`flex flex-col items-center text-xs ${isActive ? 'text-blue-600' : 'text-gray-500'}`}>
              ...
              {item.to === '/cart' && cart.length > 0 && <span className="absolute -mt-1 ml-3 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px]">{cart.length}</span>}
            </Link>
          );
        })}
      </nav>
