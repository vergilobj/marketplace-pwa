    ...
    <div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)]">
      <header className="sticky top-0 z-50 glass-strong border-b border-[var(--color-border)] shadow-[0_8px_30px_rgba(0,0,0,0.25)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center space-x-1.5 ml-4">
            <button onClick={() => setLightMode(!lightMode)} className="p-2 rounded-full hover:bg-[rgba(255,255,255,0.06)] text-[var(--color-muted)] hover:text-[#6366f1] transition-colors" title={lightMode ? 'Тёмная тема' : 'Светлая тема'}>
              {lightMode ? <Moon size={20} /> : <Sun size={20} />}
            </button>
            <Link to="/favorites" className="p-2 rounded-full hover:bg-[rgba(255,255,255,0.06)] text-[var(--color-muted)] hover:text-[#6366f1] transition-colors">
              <Heart size={20} />
            </Link>
            <Link to="/cart" className="p-2 rounded-full hover:bg-[rgba(255,255,255,0.06)] text-[var(--color-muted)] hover:text-[#6366f1] transition-colors relative">
              <ShoppingBag size={20} />
              {cart.length > 0 && (
            ...
            {isAuthenticated && (
              <>
                <Link to="/chat" className="p-2 rounded-full hover:bg-[rgba(255,255,255,0.06)] text-[var(--color-muted)] hover:text-[#6366f1] transition-colors">
                  <MessageCircle size={20} />
                </Link>
                <Link to="/notifications" className="p-2 rounded-full hover:bg-[rgba(255,255,255,0.06)] text-[var(--color-muted)] hover:text-[#6366f1] transition-colors relative">
                  <Bell size={20} />
                  {unreadCount > 0 && (
                ...
                <div className="relative" ref={profileMenuRef}>
                  <button
                    ...
                    className="p-2 rounded-full hover:bg-[rgba(255,255,255,0.06)] text-[var(--color-muted)] hover:text-[#6366f1] transition-colors"
                  >
                    <User size={20} />
                  </button>
                  {profileMenuOpen && (
        ...
        {menuOpen && (
          <div className="md:hidden px-4 pb-4 space-y-2 glass-strong">
            ...
            {!isAuthenticated && <Link to="/login" className="block py-2 text-sm text-[#6366f1]" onClick={() => setMenuOpen(false)}>Войти</Link>}
          </div>
        )}
      </header>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <div className="space-y-2">
              <Link to="/" className="block text-[var(--color-muted)] text-sm hover:text-[#6366f1] transition-colors">Главная</Link>
              <Link to="/products" className="block text-[var(--color-muted)] text-sm hover:text-[#6366f1] transition-colors">Товары</Link>
              <Link to="/favorites" className="block text-[var(--color-muted)] text-sm hover:text-[#6366f1] transition-colors">Избранное</Link>
              <Link to="/cart" className="block text-[var(--color-muted)] text-sm hover:text-[#6366f1] transition-colors">Корзина</Link>
            </div>
          </div>
          <div>
            <h4 className="font-semibold text-sm mb-3">Сервис</h4>
            <div className="space-y-2">
              <Link to="/privacy" className="block text-[var(--color-muted)] text-sm hover:text-[#6366f1] transition-colors">Приватность</Link>
              <Link to="/chat" className="block text-[var(--color-muted)] text-sm hover:text-[#6366f1] transition-colors">Чат</Link>
              <Link to="/referrals" className="block text-[var(--color-muted)] text-sm hover:text-[#6366f1] transition-colors">Рефералы</Link>
            </div>
          </div>
          <div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 glass-strong border-t border-[rgba(255,255,255,0.06)] flex justify-around py-2 z-40">
        {mobileLinks.map(item => {
          ...
          return (
            <Link key={item.to} to={item.to} className={`flex flex-col items-center text-xs ${isActive ? 'text-[#6366f1]' : 'text-[var(--color-muted)]'}`}>
