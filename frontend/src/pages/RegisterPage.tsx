  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 relative">
      <div className="absolute inset-0 -z-10 pointer-events-none overflow-hidden rounded-[34px]">
        <div className="absolute top-0 right-1/4 w-72 h-72 rounded-full blur-3xl opacity-30" style={{ background: 'radial-gradient(circle, #8ee8ff, transparent 70%)' }} />
        <div className="absolute bottom-0 left-1/4 w-72 h-72 rounded-full blur-3xl opacity-30" style={{ background: 'radial-gradient(circle, #c9f267, transparent 70%)' }} />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <div className="glass-card rounded-[34px] p-8 shadow-[0_30px_80px_rgba(0,0,0,0.55)]">
          <div className="text-center mb-8">
            <div style={{ background: 'linear-gradient(135deg, #8ee8ff 0%, #c9f267 100%)' }} className="w-14 h-14 mx-auto mb-4 rounded-2xl flex items-center justify-center shadow-[0_16px_40px_rgba(142,232,255,0.35)]"><UserPlus size={24} className="text-[#0b0e0d]" /></div>
            <h1 className="text-2xl font-bold text-[var(--color-text)] mb-1">Создать аккаунт</h1>
            <p className="text-[var(--color-muted)] text-sm flex items-center justify-center gap-1.5"><Gift size={14} className="text-[#8ee8ff]" /> Требуется код приглашения</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} style={{ background: 'linear-gradient(135deg, #8ee8ff 0%, #c9f267 100%)' }} className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-full text-[#0b0e0d] font-semibold text-sm transition-all shadow-[0_16px_40px_rgba(142,232,255,0.3)] disabled:opacity-50 hover:scale-[1.02] active:scale-[0.98]">{loading ? '...' : <><Sparkles size={16} /> Зарегистрироваться</>}</button>
          </form>

          <div className="mt-6 pt-6 border-t border-[rgba(255,255,255,0.06)] text-center">
            <p className="text-[var(--color-muted)] text-sm">Уже есть аккаунт? <Link to="/login" className="text-[#c9f267] hover:text-[#b6e04f] font-semibold">Войти</Link></p>
          </div>
        </div>
      </motion.div>
    </div>
