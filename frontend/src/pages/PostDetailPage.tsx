...
export default function PostDetailPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl overflow-hidden p-0">
        <div className="p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 text-[var(--color-text)] text-[11px] font-bold flex items-center justify-center">{(post.author?.name||post.adOwner?.name||'A')[0].toUpperCase()}</div>
          ...
          <h1 className="text-xl font-bold text-[var(--color-text)] mb-3">{post.title}</h1>
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-white/[0.06]">
            <button onClick={handleLike} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${liked?'text-rose-400 bg-rose-400/10':'text-white/60 hover:text-[var(--color-text)] hover:bg-white/[0.04]'}`}><Heart size={14} fill={liked?'currentColor':'none'}/>{likes>0&&likes}</button>
      ...
      {/* Comments */}
      <div className="mt-8"><h3 className="text-base font-bold text-[var(--color-text)] mb-4">Комментарии ({comments.length})</h3>
        <div className="space-y-2 mb-6">{comments.map((c,i)=><motion.div key={c.id} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:i*0.02}} className="glass rounded-xl p-4"><div className="flex items-start gap-3"><div className="w-7 h-7 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 text-[var(--color-text)] text-[9px] font-bold flex items-center justify-center shrink-0">{(c.user?.name||'?')[0].toUpperCase()}</div><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-1"><span className="text-xs font-semibold text-white">{c.user?.name||'Аноним'}</span><span className="text-[10px] text-white/35">{c.createdAt ? format(new Date(c.createdAt), 'd MMM, HH:mm', { locale: ru }) : ''}</span></div><p className="text-xs text-white/50">{c.text}</p></div>{(c.userId===user?.id||isAdmin)&&<button onClick={()=>delComment(c.id)} className="text-white/35 hover:text-red-400 transition-colors"><Trash2 size={13}/></button>}</div></motion.div>)}</div>
        <div className="flex gap-2"><input value={commentText} onChange={e=>setCommentText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleComment()} placeholder="Написать комментарий..." className="flex-1 px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-[var(--color-text)] text-sm placeholder:text-white/35 outline-none focus:border-indigo-500/50 transition-all" /><button onClick={handleComment} className="px-5 py-3 rounded-xl bg-indigo-500 text-[var(--color-text)] text-sm font-semibold hover:bg-indigo-400 transition-all flex items-center gap-1.5"><Send size={14}/>Отпр.</button></div>
      </div>
    </div>
  );
}
