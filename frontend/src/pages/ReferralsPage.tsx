  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      <motion.div initial={{opacity:0,scale:.97}} animate={{opacity:1,scale:1}} className="glass-card rounded-[26px] p-6 mb-6">
        ...
        <div className="flex items-center gap-2 max-w-xs mx-auto"><div className="flex-1 glass rounded-xl px-4 py-3 text-center font-mono text-lg font-bold text-[#6366f1] border border-[rgba(255,255,255,0.06)]">{profile?.referralCode||'—'}</div><button onClick={copyCode} className={`p-3 rounded-full transition-all ${copied?'bg-emerald-500 text-white':'text-[#0b0e0d]'} shadow-lg`} style={copied ? undefined : { background: 'linear-gradient(135deg, #6366f1 0%, #38bdf8 50%, #38bdf8 100%)' }}>{copied?<Check size={16}/>:<Copy size={16}/>}</button></div>
      </motion.div>
