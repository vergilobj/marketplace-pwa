            ...
            return (
              <motion.div key={product.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="p-4 flex flex-col flex-1">
                  <h3 className="text-sm font-semibold text-[var(--color-text)] line-clamp-2 mb-3 group-hover:text-indigo-400 transition-colors flex-1">{product.title}</h3>
                  ...
                  <div className="mt-auto flex gap-2" onClick={e => e.stopPropagation()}>
                    {inC ? (
                      ...
                      <button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl bg-indigo-500 text-[var(--color-text)] text-xs font-semibold hover:bg-indigo-400 transition-all"><ShoppingCart size={13} /> В корзину</button>
                    )}
