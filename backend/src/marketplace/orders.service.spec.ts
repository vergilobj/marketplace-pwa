...
describe('OrdersService', () => {
  ...
  describe('updateStatus', () => {
    it('should throw ForbiddenException if seller not owner', async () => {
      mockPrisma.order.findUnique.mockResolvedValue(mockOrder);
      await expect(
        service.updateStatus('order-1', 'other-seller', 'SELLER', {
          status: 'SHIPPED',
        }),
    ...
    it('should throw ForbiddenException if buyer not owner', async () => {
      mockPrisma.order.findUnique.mockResolvedValue(mockOrder);
      await expect(
        service.updateStatus('order-1', 'other-buyer', 'BUYER', {
          status: 'COMPLETED',
        }),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should allow admin to mark as PAID', async () => {
      ...
      const result = await service.updateStatus('order-1', 'admin', 'ADMIN', {
        status: 'PAID',
      });
