...
describe('SocialService', () => {
  ...
  const mockPrisma = {
    ...
    like: { create: jest.fn(), findUnique: jest.fn(), delete: jest.fn(), findMany: jest.fn() },
  ...
  describe('likePost', () => {
    it('should throw NotFoundException if post does not exist', async () => {
      ...
      await expect(service.likePost('user-1', 'bad-id')).rejects.toThrow(NotFoundException);
    });

    it('should like a post and notify author', async () => {
      mockPrisma.post.findUnique.mockResolvedValue({ id: 'post-1', authorId: 'author-1' });
    ...
    it('should not notify when liking own post', async () => {
      mockPrisma.post.findUnique.mockResolvedValue({ id: 'post-1', authorId: 'user-1' });
      mockPrisma.like.create.mockResolvedValue({ id: 'like-1' });
    ...
    it('should throw ConflictException on duplicate like', async () => {
      ...
      await expect(service.likePost('user-1', 'post-1')).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('unlikePost', () => {
    it('should throw NotFoundException if like not found', async () => {
      ...
      await expect(service.unlikePost('user-1', 'post-1')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('addComment', () => {
    it('should throw NotFoundException if post does not exist', async () => {
      mockPrisma.post.findUnique.mockResolvedValue(null);
      await expect(service.addComment('user-1', 'bad-id', 'Nice!')).rejects.toThrow(NotFoundException);
    });

    it('should add comment and notify', async () => {
      mockPrisma.post.findUnique.mockResolvedValue({ id: 'post-1', authorId: 'author-1' });
      mockPrisma.comment.create.mockResolvedValue({ id: 'c1', text: 'Nice!', user: { id: 'user-1', name: 'User' } });
  ...
  describe('updateComment', () => {
    it('should throw ForbiddenException if not owner and not admin', async () => {
      mockPrisma.comment.findUnique.mockResolvedValue({ id: 'c1', userId: 'owner' });
      await expect(service.updateComment('c1', 'other', 'BUYER', 'Edited'))
        .rejects.toThrow(ForbiddenException);
    });

    it('should update comment if owner', async () => {
      mockPrisma.comment.findUnique.mockResolvedValue({ id: 'c1', userId: 'user-1' });
      mockPrisma.comment.update.mockResolvedValue({ id: 'c1', text: 'Edited', user: { id: 'user-1', name: 'User' } });
      const result = await service.updateComment('c1', 'user-1', 'BUYER', 'Edited');
  ...
  describe('deleteComment', () => {
    it('should throw ForbiddenException if not owner and not admin', async () => {
      mockPrisma.comment.findUnique.mockResolvedValue({ id: 'c1', userId: 'owner' });
      await expect(service.deleteComment('c1', 'other', 'BUYER'))
        .rejects.toThrow(ForbiddenException);
    });
